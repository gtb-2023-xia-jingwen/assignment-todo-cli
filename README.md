...
![image.png](./assets/1636279046380-image.png)

修改任务标题的具体要求有：

1. 需通过参数指定要修改的任务的 ID 以及新的标题的内容；
2. 指定的任务不存在时输出`Task not found!`以提示用户；

![image.png](./assets/1636280523152-image.png)

### `remove` command

用户通过该命令删除任务，示例如下：

![image.png](./assets/1636338933717-image.png)
...
用户通过该命令将已标记为 `已完成/Completed` 的任务切换回 `待办/TBD` 状态，示例如下：

![image-20211108113136546](assets/image-20211108113136546.png)
...
![image-20211108173934181](assets/image-20211108173934181.png)

重置清单的具体要求有：

1. 清空全部任务；
2. 重置后任务 ID 重新从 1 开始生成；
3. 重置后不需要再执行 `init` command；

### Unknown command

如果输入的命令不支持，比如`todo foobar This is task name`，则提示用户`Unknown command: 'foobar'!`，示例如下图：

![image-20211111154110307](assets/image-20211111154110307.png)

在执行 `init` 之前运行任何不存在的命令，也都提示以上信息。
...
3. 批改会使用提交时间点所对应的版本，请务必在确认无误后再进行提交；
